...
- **Value**: `OBS_VALUE` is **not always a plain decimal number.** In the committed fixture, 1,155 of 1,176 rows carry a normal decimal (confirmed **percent** by inspecting magnitudes: US=3.625, GB=3.75, JP=1, XM=2.25, CH=0, and Argentina ranging 40–133 — consistent with real, extreme ARS policy rates in percent, not basis points or decimal fraction). The remaining **21 rows (all `REF_AREA`=`HR`, Croatia, spanning 2021-01 through 2022-12) carry the literal string `"NaN"`** in `OBS_VALUE` instead of a number — see the missing-data note below. A parser must not `float()` this field blindly without handling that literal.
...
- `OBS_STATUS`/`OBS_CONF`/`OBS_PRE_BREAK` are SDMX observation-quality flags. `OBS_STATUS` is **not uniformly `A`** — the fixture has `A` (1,155 rows, "actual"/normal) and `M` (21 rows, all Croatia, same rows as the `"NaN"` `OBS_VALUE`s above — "missing"). `OBS_CONF`/`OBS_PRE_BREAK` were `F`/empty across the sample. **A parser must treat literal `OBS_VALUE == "NaN"` (equivalently `OBS_STATUS == "M"`) as missing data, not as a value to parse** — do not assume this field is clean.

### Missing data and stale series — hazards for a naive "latest row" parser

Checking the last `TIME_PERIOD` present per `REF_AREA` in the fixture surfaces two distinct hazards that a parser assuming "latest row = current policy rate" would get wrong:

- **Croatia (`HR`) is effectively missing from 2021 onward.** Its series is almost entirely `OBS_VALUE`="NaN"/`OBS_STATUS`="M" from 2021-01 through 2022-12 (21 of the fixture's rows for HR), with only three real observations in between (2022-03: 0.05, 2022-04: 0, 2022-07: 0), and the series stops entirely after 2022-12. This lines up with Croatia adopting the euro on 2023-01-01 — BIS stopped publishing a distinct Croatian policy rate once the ECB rate applied there.
- **Eight legacy euro-area founding members stop dead at `1998-12`**: `AT` (Austria), `BE` (Belgium), `DE` (Germany), `ES` (Spain), `FR` (France), `IT` (Italy), `NL` (Netherlands), `PT` (Portugal) — the month before the ECB took over monetary policy on 1999-01-01. **`GR` (Greece) stops at `2000-12`**, its own euro-entry date. Every other country in the fixture (~39 of them) runs current, to 2026-06 or 2026-07.

**Hazard to spell out explicitly:** a parser that assumes "the row with the latest `TIME_PERIOD` for a given `REF_AREA` is that country's current policy rate" will silently report a **1998 Bundesbank rate as Germany's policy rate today** (and similarly stale rates for AT/BE/ES/FR/IT/NL/PT/GR). These jurisdictions' actual current policy rate lives only under the euro-area aggregate (`XM`), not under their own country code, from their euro-adoption date onward. The shipped country table for this plan does not include these legacy members today, so they are filtered out — but this is a trap for whoever widens BIS coverage later and must be handled (e.g. by excluding legacy euro members' own-country series post-adoption, or by cutting over to `XM` from their adoption date) rather than blindly trusting "latest `TIME_PERIOD` per `REF_AREA`".
...
- Candidate 2 (`home.treasury.gov/.../daily-treasury-rates.csv/2026/all?type=daily_treasury_yield_curve&...`) → **HTTP 200**, worked. This is the classic treasury.gov CSV export and it ignores any notion of "latest only" — it returned **every daily observation for calendar year 2026 to date** (1 header line + 151 data rows = 152 total lines, most recent 2026-08-07 first, descending by date).
